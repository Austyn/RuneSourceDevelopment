...
public abstract class Entity {
	...
	public int grabHitTimer() {
		...
		if (this instanceof Player) {
			...
			hitTimer = weapon == null ? 4 : weapon.getEquipmentDefintion().getHitTimer();
		} else if (this instanceof Npc) {
			Npc npc = (Npc) this;
			hitTimer = npc.getDefinition().getHitTimer();
		}
	...
	public int grabAttackAnimation() {
		...
		if (this instanceof Player) {
			...
			if (getAttackType() == AttackTypes.MAGIC) {
				anim = player.getMagic().getSpellDefinitions()[player.getMagic().getMagicIndex()].getAnimationId();
			}
			else {
				Item weapon = player.getEquipment().getItemContainer().get(3);
				anim = weapon == null ? 422 : weapon.getEquipmentDefintion().getAttackStyles().get(0);
			}
		} else if (this instanceof Npc) {
			Npc npc = (Npc) this;
			anim = npc.getDefinition().getAttackAnim();
		}
		return anim;
	}
