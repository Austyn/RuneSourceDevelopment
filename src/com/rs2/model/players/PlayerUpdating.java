...
public final class PlayerUpdating {
	...
	public static void updateState(Player player, StreamBuffer.OutBuffer block, boolean forceAppearance, boolean noChat) {
		...
		if (player.getUpdateFlags().isHitUpdate()) {
			...
			block.writeByte(player.getSkill().getLevel()[player.getSkill().HITPOINTS], StreamBuffer.ValueType.C);
			block.writeByte(player.getSkill().getLevelForXP((int) player.getSkill().getExp()[player.getSkill().HITPOINTS]));
		}
		if (player.getUpdateFlags().isHitUpdate2()) {
			...
			block.writeByte(player.getSkill().getLevel()[player.getSkill().HITPOINTS]);
			block.writeByte(player.getSkill().getLevelForXP((int) player.getSkill().getExp()[player.getSkill().HITPOINTS]), StreamBuffer.ValueType.C);
		}
	}
